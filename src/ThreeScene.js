  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        gltf.scene.traverse((child) => {
          if (child.isMesh) {
            const standardMaterial = new THREE.MeshStandardMaterial({
              color: 0xffffff,
              metalness: 1, // Metalness factor
              roughness: 0.2, // Roughness factor
        ...
        gltf.scene.scale.set(1, 1, 1); // Scale the model down
        gltf.scene.position.y = -0.7; // Move the model down by 5 units
        gltf.scene.position.x = 7; // Move the model down by 5 units
